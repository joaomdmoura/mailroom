defmodule Mailroom.IMAP do
  ...
  def connect(server, username, password, options \\ []) do
    ...
    GenServer.call(pid, {:connect, server, opts.ssl, opts.port, opts.timeout}, opts.timeout)
  ...
  def handle_call({:connect, server, ssl, port, timeout}, from, state) do
    {:ok, socket} = Socket.connect(server, port, ssl: ssl, debug: state.debug, active: true, timeout: timeout)
